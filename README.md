...
1. 从文件夹导入。
2. 等待“发现照片、解码分析、重复确认、相似分组”完成；扫描可取消，重新开始会增量复用未变化照片。
...
- 动态部分会在首次播放时转换为浏览器兼容、保留声音的 WebM 缓存。设置中可选择“不写回源文件”“每次写回时提醒”或“始终写回”；写回支持 JPEG、HEIC 与 HEIF 动态照片，会先将原图片备份到项目缓存的 `source-backups` 目录，再以经过解码和动态结构校验的临时文件原子替换源图片。配对或内嵌的视频内容不会重新编码。
- 将动态帧写回源图片后，静态图片分辨率会以动态视频帧为准，可能低于相机拍摄的原始静态照片；“每次写回时提醒”模式会在操作前明确提示这一点。
...
输出位于 `dist\Cullumi-v1.0.1\`，并生成可上传到 GitHub Release 的 `dist\Cullumi-v1.0.1-Windows-Portable.zip`。

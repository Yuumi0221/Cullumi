...
class ProjectManager:
    ...
    def open(self, root: str, cache_root: str | None = None) -> Project:
        ...
        with self.config.lock:
            existing = copy.deepcopy(self.config.data.setdefault("projects", {}).get(pid, {}))
            default_cache_root = self.config.data["default_cache_root"]
...
class Scanner:
    ...
    def _discover(self, project: Project, cancel: threading.Event) -> list[Path]:
        stored = self.config.data.get("projects", {}).get(project.project_id, {})
    ...
    def rebuild_similarity(
        ...
        def row_structure(row: sqlite3.Row) -> tuple[np.ndarray, float] | None:
            ...
            if photo_id not in structure_vectors:
                structure_vectors[photo_id] = _structure_vector(Path(row["thumbnail"]))
                ...
                for b in group[i + 1 :]:
                    ...
                    if not nearby and not sim.get("allow_cross_time_high_confidence"):
                        if seq_gap > sim["sequence_gap"]:
                            break
                        continue
                    aspect_a = derived_a["aspect"]
                    aspect_b = derived_b["aspect"]

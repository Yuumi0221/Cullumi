...
class WebStaticTests(unittest.TestCase):
    ...
    def test_similar_selection_outlines_have_safe_insets_and_subtle_scrollbars(self):
        ...
        self.assertIn("scrollbar-color: color-mix(in srgb,var(--muted) 28%,transparent) transparent;", styles)
        self.assertIn('[data-theme="night"] * {', styles)
        self.assertIn("scrollbar-color:color-mix(in srgb,var(--muted) 30%,transparent) transparent", styles)
    ...
    def test_home_uses_adaptive_golden_split_and_searchable_two_column_recents(self):
        ...
        self.assertIn("padding:37px 68px 68px 68px;", styles)
        ...
        self.assertIn("padding:10px 12px 80px 22px;", styles)
        ...
        self.assertIn(".home .hero-choose,\n[data-theme=\"night\"] .home .hero-choose {\n  width:308px;", styles)
        ...
        self.assertIn("--keep-hover:#294f3a;", styles)
    ...
    def test_warm_theme_preserves_semantic_action_colors_and_resets_similar_actions(self):
        script = (Path(__file__).parents[1] / "web" / "app.js").read_text(encoding="utf-8")
        styles = (Path(__file__).parents[1] / "web" / "app.css").read_text(encoding="utf-8")
        self.assertIn("--pink:#ca7576;", styles)
        self.assertIn("--pink2:#f4e1df;", styles)
        self.assertIn("--pink: #e59691;", styles)
        self.assertIn("--keep:#398354;", styles)
        self.assertIn("--red:#b34c41;", styles)

...
$$(".form-grid [data-p]").forEach(el=>{const b=document.createElement("button"),label=el.closest("label"),field=el.closest(".select-field")||el;b.type="button";b.className="field-reset";b.innerHTML='<svg viewBox="0 0 1024 1024" aria-hidden="true"><use transform="translate(1024 0) scale(-1 1)" href="/static/assets/icons.svg?v=1#motion-reset"></use></svg>';b.title="恢复基础模式默认值";b.setAttribute("aria-label","恢复基础模式默认值");label.classList.add("field-reset-label");label.insertBefore(b,field);b.onclick=()=>{const base=state.profiles.find(x=>x.id===(state.editor?.base_mode||"balanced"))||state.profiles.find(x=>x.id==="balanced");const v=getPath(base,el.dataset.p);if(v!==undefined){setPath(state.editor,el.dataset.p,v);el.value=v}}});

...
function photoCard(p,index,customBadge="",customKind="",extraInfo=""){
  ...
  return `<article class="photo-card ${decisionClass}" data-photo-id="${p.id}"><div class="thumb" data-open-id="${p.id}"><img loading="lazy" src="${p.thumb_url}" alt="">${p.media_type==="motion_photo"?`<span class="live-mark card-live-mark" aria-label="动态照片">${LIVE_PHOTO_ICON}</span>`:badge?`<span class="badge badge-${badgeKind}">${esc(badge)}</span>`:""}</div><div class="card-info"><b title="${esc(p.relative_path)}">${esc(p.relative_path.split("/").pop())}</b><small>${esc(p.reason||`${p.width||0}×${p.height||0} · ${formatSize(p.size||0)}`)}</small>${extraInfo?`<span class="similarity-score">${esc(extraInfo)}</span>`:""}</div><div class="card-actions"><button class="keep" data-decision="keep" data-id="${p.id}">保留</button><button class="danger" data-decision="remove" data-id="${p.id}">移除</button></div></article>`;
}
...
function syncMotionCoverMarkers(p){const wrap=$("#motionTimelineWrap"),lastFrame=lastMotionFrameTime(p),motionCover=p?.motion?.cover_source==="motion",coverTime=motionCover?Math.min(lastFrame,Math.max(0,p.motion.cover_time_ms||0)):0,percent=lastFrame?coverTime/lastFrame*100:0,edgeOffset=6-percent*0.12;wrap.style.setProperty("--motion-cover-percent",`${percent}%`);wrap.style.setProperty("--motion-cover-position",`calc(${percent}% + ${edgeOffset}px)`);$("#motionCoverMarker").title=`当前封面 · ${motionClock(coverTime)}`;$("#motionOriginalMarker").classList.toggle("hidden",!motionCover||coverTime<=0)}
...
function setupMotionViewer(p){const video=$("#viewerVideo"),lastFrame=lastMotionFrameTime(p);state.viewerMotion={active:true,scrubbing:false};$("#viewer").classList.add("viewer-motion-open");$("#motionControls").classList.remove("hidden");$("#motionTimeline").max=String(lastFrame);$("#motionTimeline").value=String(Math.min(lastFrame,p.motion.cover_source==="motion"?p.motion.cover_time_ms:0));syncMotionCoverMarkers(p);video.poster=p.photo_url;video.src=p.motion.video_url;video.classList.remove("hidden");$("#viewerImage").classList.add("hidden");resetViewerTransform();syncMotionTime();syncMotionVolume();if(!matchMedia("(prefers-reduced-motion: reduce)").matches)video.play().catch(syncMotionTime)}
async function saveMotionCover(source="motion",timeMs=null){const p=state.items[state.viewerIndex];if(!p?.motion)return;const button=source==="still"?$("#motionResetCover"):$("#motionSetCover"),requested=timeMs??Math.round($("#viewerVideo").currentTime*1000),selectedTime=source==="motion"?Math.min(lastMotionFrameTime(p),Math.max(0,requested)):0;button.disabled=true;try{const result=await json("/api/motion/cover",{project_id:state.project.id,photo_id:p.id,source,time_ms:selectedTime});state.items[state.viewerIndex]=result.photo;state.viewerNeedsRefresh=true;state.viewerDirtyIds.add(p.id);applyProjectCounts(result.project_counts);$$(`[data-photo-id="${p.id}"]`).forEach(card=>{card.querySelector("img").src=result.photo.thumb_url;card.querySelector("small").textContent=result.photo.reason||`${result.photo.width||0}×${result.photo.height||0} · ${formatSize(result.photo.size||0)}`});openViewer(state.viewerIndex);toast(source==="still"?"已恢复原始封面":"封面和照片分析已更新")}catch(error){toast(`保存封面失败：${error.message}`)}finally{button.disabled=false}}
function openViewer(i){if(!state.items.length)return;stopViewerMotion();state.viewerIndex=(i+state.items.length)%state.items.length;const p=state.items[state.viewerIndex],suggestion=viewerSuggestion(p),badge=$("#viewerBadge"),img=$("#viewerImage");resetViewerTransform();img.classList.remove("hidden");img.src=p.photo_url;$("#viewerName").textContent=p.relative_path.split("/").pop();$("#viewerMeta").textContent=`${p.width||0} × ${p.height||0} · ${formatSize(p.size||0)}${Number.isFinite(p.quality_score)?` · ${p.quality_score} 分`:""}${p.reason?" · "+p.reason:""}${p.motion?.error?" · 动态部分不可用":""}`;if(p.media_type==="motion_photo"){badge.innerHTML=LIVE_PHOTO_ICON;badge.setAttribute("aria-label","动态照片")}else{badge.textContent=suggestion.text;badge.removeAttribute("aria-label")}badge.className=`viewer-badge ${p.media_type==="motion_photo"?"viewer-live-mark":suggestion.kind?`badge-${suggestion.kind}`:"hidden"}`;updateViewerDecision(p);$("#viewerIndex").textContent=`${state.viewerIndex+1} / ${state.items.length}`;if(!$("#viewer").open)$("#viewer").showModal();if(p.motion&&!p.motion.error)setupMotionViewer(p)}

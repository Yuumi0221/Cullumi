...
function libraryPresetName() {
  if (
    ...
    setEquals(state.filters.ai, AI_VALUES)
  )
    return "library";
  if (
    setEquals(state.filters.decisions, ["undecided"]) &&
    setEquals(state.filters.ai, ["remove", "review"])
  )
    return "ai";
  if (
    setEquals(state.filters.decisions, ["undecided"]) &&
    ...
    return "undecided";
  if (
    setEquals(state.filters.decisions, ["keep"]) &&
    setEquals(state.filters.ai, AI_VALUES)
  )
    return "keep";
  if (
    setEquals(state.filters.decisions, ["remove"]) &&
    setEquals(state.filters.ai, AI_VALUES)
  )
    return "remove";
  return "library";
...
function filterQueryValue(values, allValues) {
  if (!values.size) return "none";
  if (values.size === allValues.length) return "all";
  return allValues.filter((value) => values.has(value)).join(",");
}
...
async function loadLibraryPage(reset = false) {
  ...
  if (!state.filters.decisions.size) {
    renderLibraryEmpty("当前没有选择决定状态");
    return;
  }
  if (!state.filters.ai.size) {
  ...
  try {
    ...
    if (generation !== state.library.generation || state.view !== "library")
      return;
    const start = state.items.length;
    state.items.push(...data.items);
    state.library.offset += data.items.length;
    state.library.total = data.total;
    state.library.done =
      state.library.offset >= data.total || !data.items.length;
    ...
    sentinel.textContent = state.library.done
      ? data.total
        ? "已加载全部照片"
        : ""
      : "继续向下滚动加载";
    ...
    if (generation === state.library.generation) toast(error.message);
  } finally {
    if (generation === state.library.generation) state.library.loading = false;
  }
}
async function loadView() {
  ...
  $("#viewTitle").textContent =
    state.view === "library"
      ? libraryTitle
      : { similar: "相似连拍", unreadable: "无法读取", quarantine: "隔离历史" }[
          state.view
        ];
  ...
  try {
    if (state.view === "library") {
      ...
      const d = await json(
        `/api/photos?project_id=${state.project.id}&file=unreadable&decisions=all&ai_states=all&search=${search}&limit=500`,
      );
      state.items = d.items;
      renderPhotos(d.items, d.total);
    }
  } catch (e) {
    toast(e.message);
  }
...
function renderPhotos(items, total) {
  ...
  if (empty && state.lastScan?.total === 0) {
    $("#emptyTitle").textContent = "没有发现支持的照片";
    const p = state.lastScan;
    const ext = Object.entries(p.unsupported_extensions || {})
      .sort((a, b) => b[1] - a[1])
      .map(([x, n]) => `${x} ${n} 个`)
      .join("、");
    $("#emptyText").textContent = p.video_count
      ? `此文件夹有 ${p.video_count} 个视频，但Cullumi暂不支持视频。${ext ? " 文件统计：" + ext : ""}`
      : `发现 ${p.unsupported_count || 0} 个不支持的文件。${ext}`;
  } else if (empty) {
    $("#emptyTitle").textContent = "这里还没有内容";
...
function handleGalleryClick(event) {
  ...
  if (decision) {
    ...
    setDecision(+decision.dataset.id, decision.dataset.decision, false);
    return;
  }
  ...
  if (opener) {
    const index = state.items.findIndex(
      (photo) => photo.id === +opener.dataset.openId,
    );
    if (index >= 0) openViewer(index);
  }
}
function renderBatches(items) {
  $("#viewSubtitle").textContent = `${items.length} 个批次`;
  $("#gallery").innerHTML = items
    .map(
      (x) =>
        `<article class="photo-card"><div class="card-info"><b>${esc(x.created_at)}</b><small>${x.count} 张 · ${formatSize(x.total_size)}${x.restored_at ? " · 已恢复" : ""}</small></div>${x.restored_at ? "" : `<div class="card-actions"><button data-restore="${x.id}">恢复此批次</button></div>`}</article>`,
    )
    .join("");
...
function updateCardDecision(id, decision) {
  $$(`[data-photo-id="${id}"]`).forEach((card) => {
    card.classList.toggle("decision-keep", decision === "keep");
    card.classList.toggle("decision-remove", decision === "remove");
  });
}
function reconcileLibraryDecision(id) {
  const photo = state.items.find((item) => item.id === id);
  if (!photo) return;
  if (photoMatchesLibrary(photo)) {
    updateCardDecision(id, photo.decision);
    return;
  }
  const card = $(`[data-photo-id="${id}"]`);
  if (card) card.remove();
...
async function syncViewerDecisions() {
  ...
  const ids = [...state.viewerDirtyIds];
...
function bindGalleryEvents() {
  ...
  $("#exportBtn").onclick = async () => {
    try {
      const result = await json("/api/export/save", {
        project_id: state.project.id,
      });

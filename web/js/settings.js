...
function confirmClearDecisions(){const count=Object.values(state.project?.decisions||{}).reduce((a,b)=>a+b,0);if(!count){toast("没有需要清空的选择");return}$("#confirmTitle").textContent="清空所有选择？";$("#confirmBody").textContent=`将清除当前项目中 ${count} 张照片的“保留/移除”选择，照片文件不会被移动或删除。`;$("#confirmOk").textContent="确认清空";$("#confirmOk").onclick=async()=>{try{const r=await json("/api/decision/clear",{project_id:state.project.id});$("#confirm").close();toast(`已清空 ${r.cleared} 张照片的选择`);applyProjectCounts(r.project_counts);loadView()}catch(e){toast(e.message)}};$("#confirm").showModal()}
function confirmAiRemoveSuggestions(){
  const count=state.project?.library_counts?.ai_remove_pending??0;if(!count){toast("没有未决定的建议移除照片");return}
  $("#confirmTitle").textContent=`标记 ${count} 张建议移除照片？`;
  $("#confirmBody").textContent="这些照片会统一标记为“移除”。照片文件不会立即移动，之后仍需点击“隔离已标记移除”确认处理。";
  const button=$("#confirmOk");button.textContent="全部标记移除";button.onclick=async()=>{button.disabled=true;try{const r=await json("/api/decision/ai-remove",{project_id:state.project.id});$("#confirm").close();toast(`已将 ${r.marked} 张照片标记为移除`);applyProjectCounts(r.project_counts);await loadView()}catch(e){toast(e.message)}finally{button.disabled=false}};
  $("#confirm").showModal();
}
